...
export default function ClientDetailPage() {
  ...
  async function deleteNow() {
    if (!id) return
    setError('')
    const res = await authFetch(`${API_URL}/clients/${id}`, { method: 'DELETE' })
    if (res.ok) {
      queueToast('success', 'Client supprimé.')
      router.push('/clients')
      return
    }
    const j = await res.json().catch(() => ({}))
  ...
  return (
    <AppShell title="Client">
      <ConfirmDialog
        ...
        message="Confirmer la suppression de ce client."
      ...
      {!error && !client && <div className="text-sm text-gray-600">Chargement…</div>}

      {client && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center gap-2">
            <div className="text-lg font-semibold text-gray-900">{client.name}</div>
            {client.favorite ? <span className="text-amber-600">★</span> : null}
          </div>
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-700">
            ...
            <div><span className="text-gray-500">Téléphone:</span> {client.phone || '—'}</div>
            <div className="md:col-span-2"><span className="text-gray-500">Adresse:</span> {client.address || '—'}</div>
          </div>
          <div className="mt-4">
            <div className="text-sm font-semibold text-gray-900">Notes</div>
            <div className="mt-1 text-sm text-gray-700 whitespace-pre-line">{client.notes || '—'}</div>
          </div>
        </div>
      )}
    </AppShell>
